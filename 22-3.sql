...
insert into HANGHOA VALUES('SP151407','IP7',6,'','25000','20180402',0)
...
INSERT INTO DONHANG VALUES('DH151700','SP151400','KH151600','1','KM151500','20180507','20180507','gianghai')
INSERT INTO DONHANG VALUES('DH151701','SP151401','KH151601','1','KM151500','20180507','20180507','gianghai')
INSERT INTO DONHANG VALUES('DH151702','SP151403','KH151601','2','KM151500','20180507','20180507','gianghai')
...
GO
-------------------VIEW---------------
select count(*) from hanghoa where gianhapvao = 0 
CREATE PROC PROC_TONGTIEN 
AS
BEGIN
	SELECT SUM(GIABANRA * DONHANG.SOLUONG *(CAST((100 - PHANTRAM) AS FLOAT)/CAST(100 AS float))) FROM DONHANG INNER JOIN HANGHOA ON DONHANG.MASANPHAM=HANGHOA.MASANPHAM INNER JOIN KHUYENMAI ON KHUYENMAI.MAKHUYENMAI = DONHANG.MAKHUYENMAI
END

CREATE PROC PROC_TONGTIEN1 @DAY INT,@MONTH INT,@YEAR INT
AS
BEGIN
	SELECT SUM(GIABANRA * DONHANG.SOLUONG *(CAST((100 - PHANTRAM) AS FLOAT)/CAST(100 AS float))) FROM DONHANG INNER JOIN HANGHOA ON DONHANG.MASANPHAM=HANGHOA.MASANPHAM INNER JOIN KHUYENMAI ON KHUYENMAI.MAKHUYENMAI = DONHANG.MAKHUYENMAI WHERE DAY(NGAYDAT)=@DAY AND MONTH(NGAYDAT)=@MONTH AND YEAR(NGAYDAT)=@YEAR
END

DROP PROC PROC_TONGTIEN1
EXEC PROC_TONGTIEN1 07,05,2018
CREATE PROC PROC_TONGTIENLAI
AS
BEGIN
	SELECT SUM(GIABANRA * DONHANG.SOLUONG *(CAST((100 - PHANTRAM) AS FLOAT)/CAST(100 AS float)) - GIANHAPVAO * DONHANG.SOLUONG) FROM DONHANG INNER JOIN HANGHOA ON DONHANG.MASANPHAM=HANGHOA.MASANPHAM INNER JOIN KHUYENMAI ON KHUYENMAI.MAKHUYENMAI = DONHANG.MAKHUYENMAI
END

CREATE PROC PROC_TONGTIENLAI1 @DAY INT,@MONTH INT,@YEAR INT
AS
BEGIN
	SELECT SUM(GIABANRA * DONHANG.SOLUONG *(CAST((100 - PHANTRAM) AS FLOAT)/CAST(100 AS float)) - GIANHAPVAO * DONHANG.SOLUONG) FROM DONHANG INNER JOIN HANGHOA ON DONHANG.MASANPHAM=HANGHOA.MASANPHAM INNER JOIN KHUYENMAI ON KHUYENMAI.MAKHUYENMAI = DONHANG.MAKHUYENMAI WHERE DAY(NGAYDAT)=@DAY AND MONTH(NGAYDAT)=@MONTH AND YEAR(NGAYDAT)=@YEAR
END
CREATE PROC PROC_SOHANGBANRA
AS
BEGIN
	SELECT SUM(SOLUONG) FROM DONHANG
END

CREATE PROC PROC_SOHANGBANRA1 @DAY INT,@MONTH INT,@YEAR INT
AS
BEGIN
	SELECT SUM(SOLUONG) FROM DONHANG WHERE DAY(NGAYDAT)=@DAY AND MONTH(NGAYDAT)=@MONTH AND YEAR(NGAYDAT)=@YEAR
END
drop proc PROC_SOHANGBANRA1

CREATE PROC PROC_LOINHUAN @MONTH INT,@YEAR INT
AS
BEGIN
	SELECT SUM(GIABANRA * DONHANG.SOLUONG *(CAST((100 - PHANTRAM) AS FLOAT)/CAST(100 AS float)) - GIANHAPVAO * DONHANG.SOLUONG) FROM DONHANG INNER JOIN HANGHOA ON DONHANG.MASANPHAM=HANGHOA.MASANPHAM INNER JOIN KHUYENMAI ON KHUYENMAI.MAKHUYENMAI = DONHANG.MAKHUYENMAI WHERE MONTH(NGAYDAT)=@MONTH AND YEAR(NGAYDAT)=@YEAR
END
CREATE PROC PROC_HANGBANRA @MONTH INT,@YEAR INT
AS
BEGIN
	SELECT SUM(SOLUONG) FROM DONHANG WHERE MONTH(NGAYDAT)=@MONTH AND YEAR(NGAYDAT)=@YEAR
END

create proc PROC_LUONGHANG @MONTH INT,@YEAR INT
AS
BEGIN
	SELECT COUNT(*) FROM HANGHOA WHERE MONTH(NGAYNHAP)=@MONTH AND YEAR(NGAYNHAP)=@YEAR
END